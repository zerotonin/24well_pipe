...
class VideoSplitter:
    ...
    def stabilize_frames(self, raw_centres, fps):
        raw_centres_arr = np.array(raw_centres)
        
        # Gaussian filter
        smoothed_centres_gaussian = self.gaussian_filter(raw_centres_arr.squeeze(), fps)
    ...
    def create_and_write_subframes(self, path_to_video, path_for_output,vid_capture,frame_limit):
        ...
        sr = StackReg(StackReg.TRANSLATION) # initialise stabiliser
        ...
        with tqdm(total=n_frames_to_process, desc="Writing Individual Wells", unit="frame") as pbar:
            while(vid_capture.isOpened()):
                ...
                if ret and framecounter<frame_limit:
                    ...
                    if framecounter==0:
                        reference_subframes=subframes
                    for i, subframe in enumerate(subframes):
                        if framecounter!=0:
                            ref = reference_subframes[i]
                            subframe=sr.register_transform(ref, subframe)
